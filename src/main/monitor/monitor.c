...
void display_general_information_values( WINDOW* window, memory_t* mem )
{
    ...
    elapsed_time = mem->elapsed_time;
    simulation_has_ended = mem->simulation_has_ended;
    hour = (int)mem->simulationTime;
    minutes = (int)round( ( mem->simulationTime - (double)hour ) * 60. );
    /* ---------------------------------------------------------------------- */

    mvwprintw( window, 20, 8, "%.2f s", elapsed_time / 1000000 );
...
void display_citizen_information( WINDOW* window, memory_t* mem, int row, int column )
{
    ...
    number_of_citizens_at_home = mem->characters.citizens_state[INDEX_HOME];
    number_of_citizens_at_work = mem->characters.citizens_state[INDEX_WORK];
    number_of_citizens_walking = mem->characters.citizens_state[INDEX_MOVING];
    /* ---------------------------------------------------------------------- */
...
void display_spy_information( WINDOW* window, memory_t* mem, int row, int column, int number )
{
    ...
    char stolen_message_content[MAX_MESSAGE_LENGTH];

    id = mem->characters.spies[number].id;
    health_points = mem->characters.spies[number].health_points;
    location_row = mem->characters.spies[number].location.row;
    location_column = mem->characters.spies[number].location.column;
    home_row = mem->characters.spies[number].home.row;
    home_column = mem->characters.spies[number].home.column;
    nb_of_stolen_companies = mem->characters.spies[number].nb_of_stolen_companies;
    has_license_to_kill = mem->characters.spies[number].has_license_to_kill;
    strcpy( stolen_message_content, mem->characters.spies[number].stolen_message.content );
    /* ---------------------------------------------------------------------- */
...
void display_case_officer_information( WINDOW* window, memory_t* mem, int row, int column )
{
    ...
    id = mem->characters.case_officers[0].id;
    health_points = mem->characters.case_officers[0].health_points;
    location_row = mem->characters.case_officers[0].location.row;
    location_column = mem->characters.case_officers[0].location.column;
    home_row = mem->characters.case_officers[0].home.row;
    home_column = mem->characters.case_officers[0].home.column;
    mailbox_row = mem->characters.case_officers[0].mailbox.row;
    mailbox_column = mem->characters.case_officers[0].mailbox.column;
    /* ---------------------------------------------------------------------- */
...
void display_counterintelligence_officer_information( WINDOW* window, memory_t* mem, int row, int col )
{
    ...
    id = mem->characters.counter_officers[0].id;
    health_points = mem->characters.counter_officers[0].health_points;
    location_row = mem->characters.counter_officers[0].location.row;
    location_column = mem->characters.counter_officers[0].location.column;
    city_hall_row = mem->characters.counter_officers[0].city_hall.row;
    city_hall_column = mem->characters.counter_officers[0].city_hall.column;
    mailbox_row = mem->characters.counter_officers[0].mailbox_location.row;
    mailbox_column = mem->characters.counter_officers[0].mailbox_location.column;
    targeted_character_id = mem->characters.counter_officers[0].targeted_character_id;
    /* ---------------------------------------------------------------------- */
...
void display_mailbox_content( WINDOW* window, memory_t* mem )
{
    ...
    mailbox_nb_of_msgs = mem->mailbox_nb_of_msgs;
    priority = mem->mailbox_content->priority;
    /* ---------------------------------------------------------------------- */
...
void display_enemy_country_monitor( WINDOW* window )
{
    ...
    fptr = fopen( "messages_received.txt", "r" );
    if ( fptr == NULL )
    {
        return;
    }

    while ( fgets( buffer, MAX_MESSAGE_LENGTH, fptr ) != NULL )
    {
